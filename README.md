...
### 本を移動・更新した後も読書データを引き継ぐ

- EPUBを移動・改名した後や、同じファイル名の更新版へ差し替えた後も、読書位置、しおり、本ごとの読書設定、読書履歴を引き継ぎます。
- ホームの本の履歴で同じ本が重複しにくくなり、既読・完読アイコンも再起動後に維持されます。
- 書庫へ移した本は、長押しメニューから元の場所へ戻せます。同じ名前の本がある場合も、既存ファイルを上書きしません。

### Unicode基準の縦書き

- 縦書きの文字方向を [UAX #50](https://www.unicode.org/reports/tr50/) の既定値に合わせました。丸数字・ローマ数字・温度記号などは直立し、矢印・ギリシャ文字・キリル文字・欧文は横倒しになります。
- 縦書きレイアウトの変更により、v0.7.1でEPUBを初めて開く際は、対象の読書キャッシュが自動で再生成されます。読書位置、しおり、本ごとの設定は保持されます。
