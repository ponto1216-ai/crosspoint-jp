...
- SDカードフォントの導入とダウンロードを安定化しました。 [Issue #35](https://github.com/ponto1216-ai/crosspoint-jp/issues/35)
  - 互換配置を含めてインストール済みフォントを再検出し、一覧表示を軽くしました。
  - 通信が途切れたダウンロードは途中ファイルを残し、続きから再試行できます。
  - 低メモリ時の通信を安定化し、ダウンロード中もスクリーンショットを保存できます。
- EPUB読書とキャッシュ生成のメモリ処理を改善しました。 [Issue #36](https://github.com/ponto1216-ai/crosspoint-jp/issues/36) / [Issue #37](https://github.com/ponto1216-ai/crosspoint-jp/issues/37)
  - WebUIまたはCalibreで転送したEPUBを、再起動せずに開けるようメモリを回復します。
  - X3では、長い章の解析中と章の切り替え時にSDフォントの作業用メモリを回収し、一括生成終盤のクラッシュを防ぎます。
  - X4では、低メモリ時に危険な解析を続けず、対象の本を安全に中断して次へ進めます。
