> 過去資料: v0.7.3時点の `USER_GUIDE.md` を保存しています。現行の操作・仕様には使用しないでください。
...
See the [webserver docs](../../webserver.md) for more information on how to connect to the web server and upload files.

> [!TIP]
> Advanced users can also manage files programmatically or via the command line using `curl`. See the [webserver docs](../../webserver.md) for details.
